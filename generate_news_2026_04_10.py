...
# 保存文件
output_path = f'news_data/news_{date}.json'
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(news_data, f, ensure_ascii=False, indent=2)

print(f"✓ 已生成: {output_path}")
print(f"  - all_news: {len(all_news)} 条")
print(f"  - s_level: {len(s_level)} 条")
print(f"  - a_level: {len(a_level)} 条")
print(f"  - douyin: {len(douyin)} 条")
print(f"  - wukong_judgment: {len(wukong_judgment)} 字符")
print(f"  - bajie_conclusion: {len(bajie_conclusion)} 字符")